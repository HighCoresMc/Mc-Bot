...
public class CrateDropCommand extends ListenerAdapter {
    ...
    public void onModalInteraction(@NotNull ModalInteractionEvent event) {
        ...
        synchronized (challenge) {
            ...
            if (!challenge.isSolving || challenge.isDecoding || challenge.solved) {
                event.reply("❌ لقد انتهى الوقت المخصص للحل أو تم إلغاء المحاولة.").setEphemeral(true).queue();
                return;
            }
        ...
        try {
            ...
            DrawTextAligned drawTextRightAligned = (text, rightEdge, centerY) -> {
                try {
                    ...
                    // Added 80 pixels to push it to the right exactly as requested
                    float x = rightEdge - visualWidth + 80;
            ...
            // Right edges are set 15px to the left of the titles
            // Prize Title is at 1453 -> Right edge = 1438
            // Level Title is at 1398 -> Right edge = 1383
            // Status Title is at 1465 -> Right edge = 1450
            drawTextRightAligned.draw(prizeText, 1438, 484);
            drawTextRightAligned.draw(getLevelText(level), 1383, 588);
            drawTextRightAligned.draw(statusText, 1450, 695);
    ...
    private void handleClaimClick(ButtonInteractionEvent event, int historyId) {
        ...
        synchronized (challenge) {
            ...
            challenge.lockedUntil = System.currentTimeMillis() + 60000;
        }
        ...
        event.deferReply(true).queue(hook -> {
            ...
            synchronized (challenge) {
                if (!userId.equals(challenge.lockedByUserId)) {
                    hook.sendMessage("❌ انتهت صلاحية المحاولة أو تم إلغاؤها.").queue();
                    return;
                }
        ...
        animTask[0] = scheduler.scheduleAtFixedRate(() -> {
            try {
                ...
                synchronized (challenge) {
                    ...
                    lockedUser = challenge.lockedByUserId;
                    prize = challenge.prize;
                    command = challenge.command;
                    level = challenge.level;
                    p = progress.getAndAdd(20);
                }
    ...
    public static class CrateChallenge {
        ...
        public ScheduledFuture<?> transitionTask;
