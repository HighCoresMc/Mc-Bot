...
public class TeamCommand extends ListenerAdapter {
    ...
    private static class AllianceVoteState {
        ...
        AllianceVoteState(int rId, int tId, String rName, String tName,
            ...
            requesterTeamId = rId;
            targetTeamId = tId;
            requesterTeamName = rName;
            targetTeamName = tName;
            eligibleVoters = eligible;
            requesterCmdChannelId = rCmd;
            targetCmdChannelId = tCmd;
            firstLeaderId = fLdr;
            secondLeaderId = sLdr;
        }
    }
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    try (PreparedStatement upd = conn
                            .prepareStatement("UPDATE teams SET tag = 'Approved' WHERE id = ?")) {
                        upd.setInt(1, teamId);
                        upd.executeUpdate();
                    }
                    com.highcore.bot.database.SupabaseManager supa = LeonTrotskyBot.getSupabaseManager();
                    if (supa != null)
                        supa.updateTeamTag(teamName, "Approved");
    ...
    private void showDashboard(SlashCommandInteractionEvent event) {
        ...
        List<TeamData> teams = getTeamList();
        StringBuilder sb = new StringBuilder("## 🏆 لوحة تحكم الفرق\n\n**إجمالي الفرق:** `").append(count)
                .append("`\n\n");
        if (!teams.isEmpty())
            for (TeamData t : teams)
                sb.append(tagEmoji(t.tag)).append(" **").append(t.name).append("** | 🎨 `").append(t.color)
                        .append("` | 🏷️ `").append(t.tag).append("`\n");
        else
    ...
    private void handleTopCommand(SlashCommandInteractionEvent event) {
        ...
        List<TeamData> teams = getTeamList();
        if (teams.isEmpty()) {
            event.getHook().editOriginal("❌ لا توجد فرق مسجلة حالياً.").queue();
            return;
        }

        // Fetch stats and score teams
        record TeamScore(TeamData team, LiveTeamStats stats, int score) {}
        List<TeamScore> scores = new java.util.ArrayList<>();
        for (TeamData t : teams) {
            LiveTeamStats stats = fetchLiveTeamStats(t.name);
            int points = stats.points == -1 ? 0 : stats.points;
            int claims = stats.claims == -1 ? 0 : stats.claims;
            int gens = stats.generators == -1 ? 0 : stats.generators;
            int level = stats.level == -1 ? 0 : stats.level;
            // Scoring formula: points + (level * 100) + (claims * 10) + (gens * 50)
            int score = points + (level * 100) + (claims * 10) + (gens * 50);
            scores.add(new TeamScore(t, stats, score));
        }
        ...
        for (int i = 0; i < scores.size() && i < 10; i++) {
            TeamScore ts = scores.get(i);
            String rankIcon = i < 3 ? medals[i] : "🏅";
            
            sb.append(rankIcon).append(" **").append(ts.team.name).append("**\n");
            sb.append("> 📊 **التقييم الشامل:** `").append(ts.score).append("` نقطة\n");
            sb.append("> 🏰 **الأراضي:** `").append(ts.stats.claims == -1 ? 0 : ts.stats.claims).append("` | ");
            sb.append("⚡ **المولدات:** `").append(ts.stats.generators == -1 ? 0 : ts.stats.generators).append("`\n");
            if (ts.stats.level != -1) {
    ...
    private void handleCreate(SlashCommandInteractionEvent event) {
        ...
        int bannedNum = -1;
        if (leader.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 1;
        else if (member2.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 2;
        else if (member3 != null && member3.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 3;
        else if (member4 != null && member4.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 4;

        if (bannedNum != -1) {
            sendLog(event.getGuild(), "/team create (Blocked)", event.getUser(), "Team System",
                    "### 🚫 محاولة إنشاء فريق بعضو محظور\n▫️ **اسم الفريق:** " + teamName
                            + "\n▫️ **العضو المحظور:** رقم " + bannedNum,
                    "#ff0000");
    ...
    private int saveTeamToDb(String name, String color, String roleId, String catId, String voiceId,
        ...
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ...
                ps.setString(7, cmdId);
                ps.setString(8, logId);
                ps.setString(9, lId);
                ps.setString(10, m2);
                ps.setString(11, m3);
                ps.setString(12, m4);
    ...
    private void saveLogoToDb(int teamId, String url) {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
            ...
            logger.error("Error saving logo to DB", e);
        }
    }
    ...
    private void handleEdit(SlashCommandInteractionEvent event) {
        ...
        if (teamOpt != null && !teamOpt.isEmpty()) {
            ...
            event.replyComponents(Container.of(TextDisplay.of("## ✏️ تعديل فريق\nاختر الفريق:"),
                    Separator.createDivider(Separator.Spacing.SMALL), ActionRow.of(menu.build()))).useComponentsV2(true)
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (id.startsWith("tm_ally_accept_")) {
            handleAllianceVote(event, id.replace("tm_ally_accept_", ""), true);
            return;
        }
        if (id.startsWith("tm_ally_reject_")) {
            handleAllianceVote(event, id.replace("tm_ally_reject_", ""), false);
            return;
        }
        ...
        if (id.startsWith("tm_ldr_")) {
            handleLeaderPanelButton(event, id, member);
            return;
        }
    ...
    private void handleLeaderPanelButton(ButtonInteractionEvent event, String id, Member member) {
        ...
        if (id.startsWith("tm_ldr_war_")) {
            ...
            if (!isLeader) {
                event.reply("❌ فقط الليدر يمكنه تعيين الكو-ليدر.").setEphemeral(true).queue();
                return;
            }
            handleSetCoLeader(event, Integer.parseInt(id.replace("tm_ldr_coset_", "")));
        } else if (id.startsWith("tm_ldr_color_")) {
            if (!isLeader) {
                event.reply("❌ فقط الليدر يمكنه تغيير اللون.").setEphemeral(true).queue();
                return;
            }
        ...
        } else if (id.startsWith("tm_ldr_name_")) {
            if (!isLeader) {
                event.reply("❌ فقط الليدر يمكنه تغيير الاسم.").setEphemeral(true).queue();
                return;
            }
            handleNameChangeButton(event, Integer.parseInt(id.replace("tm_ldr_name_", "")));
        }
    }
    ...
    private void handlePanelList(ButtonInteractionEvent event) {
        ...
        for (TeamData t : teams)
            sb.append(tagEmoji(t.tag)).append(" **").append(t.name).append("** | 🎨 `").append(t.color).append("`\n");
        StringSelectMenu.Builder menu = StringSelectMenu.create("tm_select_view")
                .setPlaceholder("اختر فريقاً لعرض تفاصيله...");
        teams.stream().limit(25).forEach(t -> menu.addOption(t.name, String.valueOf(t.id), t.tag));
        event.replyComponents(Container.of(TextDisplay.of(sb.toString()),
                Separator.createDivider(Separator.Spacing.SMALL), ActionRow.of(menu.build()))).useComponentsV2(true)
                .setEphemeral(true).queue();
    }
    ...
    private void handlePanelDelete(ButtonInteractionEvent event) {
        ...
        event.replyComponents(
                Container.of(TextDisplay.of("## 🗑️ حذف فريق\n⚠️ **تحذير:** هذه العملية لا يمكن التراجع عنها!"),
                        Separator.createDivider(Separator.Spacing.SMALL), ActionRow.of(menu.build())))
                .useComponentsV2(true).setEphemeral(true).queue();
    }
    ...
    private void handleConfirmDelete(ButtonInteractionEvent event, int teamId) {
        ...
        event.replyModal(Modal.create("tm_modal_del_" + teamId, "سبب حذف الفريق")
                .addComponents(Label.of("السبب", reasonInput)).build()).queue();
    }
    ...
    private void handleEditButton(ButtonInteractionEvent event, int teamId) {
        ...
        TextInput nameInput = TextInput.create("tm_name", TextInputStyle.SHORT).setValue(td.name).setMaxLength(50)
                .setRequired(true).build();
        TextInput colorInput = TextInput.create("tm_color", TextInputStyle.SHORT).setValue(td.color).setMaxLength(20)
                .setRequired(true).build();
        event.replyModal(Modal.create("tm_modal_edit_" + teamId, "تعديل الفريق: " + td.name)
                .addComponents(Label.of("اسم الفريق الجديد", nameInput),
    ...
    private void handleEditMembersButton(ButtonInteractionEvent event, int teamId) {
        ...
        TextInput.Builder lB = TextInput.create("tm_leader", TextInputStyle.SHORT)
                .setPlaceholder("@mention أو Discord ID").setRequired(true);
        TextInput.Builder m2B = TextInput.create("tm_m2", TextInputStyle.SHORT).setPlaceholder("@mention (فارغ للحذف)")
                .setRequired(false);
        TextInput.Builder m3B = TextInput.create("tm_m3", TextInputStyle.SHORT).setPlaceholder("@mention (فارغ للحذف)")
                .setRequired(false);
        TextInput.Builder m4B = TextInput.create("tm_m4", TextInputStyle.SHORT).setPlaceholder("@mention (فارغ للحذف)")
                .setRequired(false);
        if (td.leaderId != null && !td.leaderId.isEmpty())
            lB.setValue("<@" + extractIdOnly(td.leaderId) + ">");
        if (td.member2Id != null && !td.member2Id.isEmpty())
            m2B.setValue("<@" + extractIdOnly(td.member2Id) + ">");
        if (td.member3Id != null && !td.member3Id.isEmpty())
            m3B.setValue("<@" + extractIdOnly(td.member3Id) + ">");
        if (td.member4Id != null && !td.member4Id.isEmpty())
            m4B.setValue("<@" + extractIdOnly(td.member4Id) + ">");
        event.replyModal(Modal.create("tm_modal_members_" + teamId, "تعديل أعضاء: " + td.name)
                .addComponents(Label.of("القائد (Leader)", lB.build()), Label.of("العضو الثاني", m2B.build()),
                        Label.of("العضو الثالث (اختياري)", m3B.build()),
                        Label.of("العضو الرابع (اختياري)", m4B.build()))
                .build()).queue();
    }
    ...
    private void handleSetCoLeader(ButtonInteractionEvent event, int teamId) {
        ...
        if (memberIds.isEmpty()) {
            event.reply("❌ لا يوجد أعضاء لتعيينهم كو-ليدر.").setEphemeral(true).queue();
            return;
        }
        ...
        guild.retrieveMembersByIds(longIds).onSuccess(members -> {
            StringSelectMenu.Builder menu = StringSelectMenu.create("tm_ldr_coset_select_" + teamId)
                    .setPlaceholder("اختر العضو لتعيينه كو-ليدر...");
            for (String uid : memberIds) {
                ...
                net.dv8tion.jda.api.components.selections.SelectOption opt = net.dv8tion.jda.api.components.selections.SelectOption
                        .of(label, uid);
                if (m != null)
                    opt = opt.withDescription("@" + m.getUser().getName());
                menu.addOptions(opt);
            }
            if (td.coLeaderId != null && !td.coLeaderId.isEmpty())
                menu.addOption("🗑️ إزالة الكو-ليدر الحالي", "remove_co");
            event.getHook()
                    .editOriginalComponents(Container.of(TextDisplay.of("## 🥈 تعيين كو-ليدر\nاختر العضو:"),
                            Separator.createDivider(Separator.Spacing.SMALL), ActionRow.of(menu.build())))
    ...
    private void handleCoLeaderSelected(StringSelectInteractionEvent event, int teamId) {
        ...
        if (td.coLeaderId != null && !td.coLeaderId.isEmpty()) {
            guild.retrieveMemberById(td.coLeaderId).queue(m -> {
                if (coRole != null)
                    guild.removeRoleFromMember(m, coRole).queue(null, e -> {
                    });
            }, e -> {
            });
            String oldCo = getMcName(td.coLeaderId);
            if (oldCo != null)
                ptero.sendConsoleCommand("teama demote " + oldCo);
        }
        if ("remove_co".equals(selectedId)) {
            updateCoLeaderInDb(teamId, null);
            event.reply("✅ تم إزالة الكو-ليدر.").setEphemeral(true).queue();
            return;
        }
        guild.retrieveMemberById(selectedId).queue(m -> {
            if (coRole != null)
                guild.addRoleToMember(m, coRole).queue(null, e -> {
                });
        }, e -> {
        });
        ...
        if (newCo != null)
            ptero.sendConsoleCommand("teama promote " + newCo);
    ...
    private void updateCoLeaderInDb(int teamId, String userId) {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
            ...
            logger.error("Error updating co-leader in DB", e);
        }
    }
    ...
    private void handleColorChangeButton(ButtonInteractionEvent event, int teamId) {
        ...
        TextInput colorInput = TextInput.create("tm_ldr_color_val", TextInputStyle.SHORT).setValue(td.color)
                .setMaxLength(20).setRequired(true).build();
        event.replyModal(Modal.create("tm_modal_ldr_color_" + teamId, "تغيير لون تيم " + td.name)
                .addComponents(Label.of("كود اللون الجديد (مثال: #FF5733)", colorInput)).build()).queue();
    }
    ...
    private void handleColorModal(ModalInteractionEvent event, int teamId) {
        ...
        Color color;
        try {
            color = Color.decode(newColor.startsWith("#") ? newColor : "#" + newColor);
        } catch (Exception e) {
        ...
        if (td.roleId != null) {
            ...
            if (role != null)
                role.getManager().setColor(color).queue(null, e -> {
                });
        }
        ...
        updateAnnouncementEmbed(guild, teamId, td.name, extractIdOnly(td.leaderId), extractIdOnly(td.member2Id),
    ...
    private void handleNameChangeButton(ButtonInteractionEvent event, int teamId) {
        TeamData td = getTeamById(teamId);
        if (td == null) {
            event.reply("❌ الفريق غير موجود.").setEphemeral(true).queue();
            return;
        }
        TextInput nameInput = TextInput.create("tm_ldr_name_val", TextInputStyle.SHORT).setValue(td.name)
                .setMaxLength(50).setRequired(true).build();
        event.replyModal(Modal.create("tm_modal_ldr_name_" + teamId, "تغيير اسم تيم " + td.name)
                .addComponents(Label.of("الاسم الجديد للتيم", nameInput)).build()).queue();
    }

    private void handleNameModal(ModalInteractionEvent event, int teamId) {
        String newName = event.getValue("tm_ldr_name_val").getAsString().trim();
        event.deferReply().setEphemeral(true).queue();
        TeamData td = getTeamById(teamId);
        if (td == null) {
            event.getHook().editOriginal("❌ الفريق غير موجود.").queue();
        ...
        if (td.name.equals(newName)) {
            event.getHook().editOriginal("❌ الاسم الجديد مطابق للاسم القديم.").queue();
            return;
        }
        ...
        if (td.roleId != null) {
            Role role = guild.getRoleById(td.roleId);
            if (role != null)
                role.getManager().setName(newName).queue(null, e -> {});
        }
        if (td.categoryId != null) {
            Category cat = guild.getCategoryById(td.categoryId);
            if (cat != null)
                cat.getManager().setName("〔🏰〕" + newName).queue(null, e -> {});
        }

        String oldName = td.name;
        ...
        syncToSupabase(newName, td.color, td.leaderId, td.member2Id, td.member3Id, td.member4Id, td.tag, oldName);
        updateAnnouncementEmbed(guild, teamId, newName, extractIdOnly(td.leaderId), extractIdOnly(td.member2Id),
                extractIdOnly(td.member3Id), extractIdOnly(td.member4Id), td.color);
        com.highcore.bot.services.TeamLogService.logEvent(newName, "✏️ تغيير الاسم", "تم تغيير اسم التيم من " + oldName + " إلى " + newName, java.awt.Color.CYAN);
        event.getHook().editOriginal("✅ تم تغيير اسم التيم إلى `" + newName + "`!").queue();
    }
    ...
    private void handleWarSelectTarget(ButtonInteractionEvent event, int teamId) {
        ...
        if (teams.isEmpty()) {
            event.reply("❌ لا توجد أتيام أخرى.").setEphemeral(true).queue();
            return;
        }
        StringSelectMenu.Builder menu = StringSelectMenu.create("tm_war_select_" + teamId)
                .setPlaceholder("اختر التيم الذي ستحارب...");
    ...
    private void handleWarTargetSelected(StringSelectInteractionEvent event, int attackerTeamId) {
        ...
        TextInput reasonInput = TextInput.create("tm_war_reason", TextInputStyle.PARAGRAPH)
                .setPlaceholder("اكتب سبب إعلان الحرب...").setMinLength(10).setRequired(true).build();
        event.replyModal(Modal.create("tm_modal_war_" + attackerTeamId + "_" + targetTeamId, "سبب إعلان الحرب")
                .addComponents(Label.of("سبب الحرب", reasonInput)).build()).queue();
    }
    ...
    private void handleWarModal(ModalInteractionEvent event, int attackerTeamId, int targetTeamId) {
        ...
        if (attacker == null || target == null) {
            event.getHook().editOriginal("❌ خطأ في بيانات الأتيام.").queue();
            return;
        }
        TextChannel warCh = event.getGuild().getTextChannelById(WAR_CHANNEL_ID);
        if (warCh == null) {
            event.getHook().editOriginal("❌ قناة الحرب غير موجودة.").queue();
            return;
        }
        ...
        String part1 = "### إعلان حرب رسمي\n\n**بِسْمِ السُّلطةِ والعَهْد**\n\n" +
                ...
                "بموجب الصلاحيات الممنوحة إلى مجلس تيم **" + attacker.name
                + "**، وبعد النظر في المواقف والأحداث القائمة بين الطرفين، فقد صدر القرار الآتي:\n\n" +
                "# **إعلان حالة الحرب**\n\n" +
                "يعلن تيم **" + attacker.name + "** رسميًا دخوله في حالة حرب مع تيم **" + target.name
                + "**، ابتداءً من:\n\n`" + date + "`\n\n" +
                "**سبب إعلان الحرب**\n> " + reason + "\n\n**قائد التيم المُعلن**\n`" + leaderMention + "`\n\n" +
        ...
        warCh.sendMessage(
                new MessageCreateBuilder()
                        .setComponents(Container.of(TextDisplay.of(part1),
                                Separator.createDivider(Separator.Spacing.LARGE), TextDisplay.of(part2)))
                        .useComponentsV2(true).build())
                .queue();

        String lMc1 = getMcName(attacker.leaderId);
        String lMc2 = getMcName(target.leaderId);
        if (lMc1 != null && lMc2 != null) {
            ptero.sendConsoleCommand("execute as " + lMc1 + " run team enemy " + target.name);
            ptero.sendConsoleCommand("execute as " + lMc2 + " run team enemy " + attacker.name);
        }

        com.highcore.bot.services.TeamLogService.logEvent(attacker.name, "⚔️ إعلان حرب", "تم إعلان الحرب على تيم " + target.name, java.awt.Color.RED);
        com.highcore.bot.services.TeamLogService.logEvent(target.name, "⚔️ إعلان حرب", "تيم " + attacker.name + " أعلن الحرب عليكم!", java.awt.Color.RED);
    ...
    private void handleAllianceSelectTarget(ButtonInteractionEvent event, int teamId) {
        ...
        StringSelectMenu.Builder menu = StringSelectMenu.create("tm_ally_select_" + teamId)
                .setPlaceholder("اختر التيم الذي تريد التحالف معه...");
        teams.stream().limit(25).forEach(t -> menu.addOption(t.name, String.valueOf(t.id)));
    ...
    private void handleAllianceTargetSelected(StringSelectInteractionEvent event, int requesterTeamId) {
        ...
        if (requester == null || target == null) {
            event.reply("❌ خطأ في بيانات الأتيام.").setEphemeral(true).queue();
            return;
        }
        if (target.cmdChannelId == null) {
            event.reply("❌ التيم المستهدف لا يملك روم CMD.").setEphemeral(true).queue();
            return;
        }
        Guild guild = event.getGuild();
        TextChannel cmdCh = guild.getTextChannelById(target.cmdChannelId);
        if (cmdCh == null) {
            event.reply("❌ لم يتم العثور على روم CMD للتيم.").setEphemeral(true).queue();
            return;
        }
        ...
        StringBuilder inv = new StringBuilder("## 🤝 طلب تحالف\n\n")
                .append("تيم **").append(requester.name).append("** يطلب التحالف مع تيم **").append(target.name)
                .append("**!\n\n")
                .append("**الأعضاء المطلوب تصويتهم:** (يلزم ").append(majority).append(" من أصل ").append(total)
                .append(")\n");
        for (String id : eligibleIds)
            inv.append("> <@").append(id).append(">\n");
        inv.append("\n**أصوات القبول ✅**\n> *لا يوجد بعد*\n\n**أصوات الرفض ❌**\n> *لا يوجد بعد*\n\n`0/").append(total)
                .append("` تم التصويت");

        cmdCh.sendMessage(new MessageCreateBuilder()
                .setComponents(
                        Container.of(TextDisplay.of(inv.toString()), Separator.createDivider(Separator.Spacing.SMALL),
                                ActionRow.of(Button.success("tm_ally_accept_" + voteId, "✅ قبول"),
                                        Button.danger("tm_ally_reject_" + voteId, "❌ رفض"))))
                .useComponentsV2(true).build())
    ...
    private void handleAllianceVote(ButtonInteractionEvent event, String voteId, boolean accepted) {
        ...
        if (state == null) {
            event.reply("❌ انتهت جلسة التصويت.").setEphemeral(true).queue();
            return;
        }
        String userId = event.getMember().getId();
        if (!state.eligibleVoters.contains(userId)) {
            event.reply("❌ أنت لست من أعضاء هذا التيم.").setEphemeral(true).queue();
            return;
        }

        state.acceptVoters.remove(userId);
        state.rejectVoters.remove(userId);
        if (accepted)
            state.acceptVoters.add(userId);
        else
            state.rejectVoters.add(userId);

        int total = state.eligibleVoters.size(), accepts = state.acceptVoters.size(),
                rejects = state.rejectVoters.size(), majority = total / 2 + 1;
        updateAllianceVoteEmbed(event.getGuild(), state, voteId, total, accepts, rejects);
        event.deferEdit().queue();

        if (accepts >= majority) {
            allianceVotes.remove(voteId);
            finalizeAlliance(event.getGuild(), state, true);
        } else if (rejects >= majority || (accepts + rejects == total)) {
            allianceVotes.remove(voteId);
            finalizeAlliance(event.getGuild(), state, false);
        }
    }

    private void updateAllianceVoteEmbed(Guild guild, AllianceVoteState state, String voteId, int total, int accepts,
            int rejects) {
        StringBuilder sb = new StringBuilder("## 🤝 طلب تحالف\n\n")
                .append("تيم **").append(state.requesterTeamName).append("** ↔ تيم **").append(state.targetTeamName)
                .append("**\n\n")
                .append("**الأعضاء:** (يلزم ").append(total / 2 + 1).append(" من أصل ").append(total).append(")\n");
        for (String id : state.eligibleVoters)
            sb.append("> <@").append(id).append(">\n");
        sb.append("\n**أصوات القبول ✅**\n");
        if (state.acceptVoters.isEmpty())
            sb.append("> *لا يوجد بعد*\n");
        else
            for (String id : state.acceptVoters)
                sb.append("> <@").append(id).append(">\n");
        sb.append("\n**أصوات الرفض ❌**\n");
        if (state.rejectVoters.isEmpty())
            sb.append("> *لا يوجد بعد*\n");
        else
            for (String id : state.rejectVoters)
                sb.append("> <@").append(id).append(">\n");
        sb.append("\n`").append(accepts + rejects).append("/").append(total).append("` تم التصويت");

        Container updated = Container.of(TextDisplay.of(sb.toString()),
                Separator.createDivider(Separator.Spacing.SMALL),
                ActionRow.of(Button.success("tm_ally_accept_" + voteId, "✅ قبول"),
                        Button.danger("tm_ally_reject_" + voteId, "❌ رفض")));
        if (state.voteMessageId != null && state.targetCmdChannelId != null) {
            ...
            if (ch != null)
                ch.editMessageById(state.voteMessageId,
                        new MessageEditBuilder().setComponents(updated).useComponentsV2(true).build())
                        .queue(null, e -> {
                        });
        }
    }
    ...
    private void finalizeAlliance(Guild guild, AllianceVoteState state, boolean accepted) {
        if (accepted) {
            ...
            String part1 = "### إعلان تحالف رسمي\n\n**بِسْمِ الوَحْدَةِ والوَفاء**\n\n" +
                    ...
                    "بناءً على ما تم الاتفاق عليه بين قيادة تيم **" + state.requesterTeamName + "** وقيادة تيم **"
                    + state.targetTeamName + "**، فقد تم إبرام ميثاق رسمي يجمع الطرفين تحت عهد واحد.\n\n" +
                    "# **إعلان قيام تحالف**\n\n" +
                    "أصبح تيم **" + state.requesterTeamName + "** وتيم **" + state.targetTeamName
                    + "** حليفين رسميًا ابتداءً من:\n\n`" + date + "`\n\n" +
            ...
            if (allyCh != null)
                allyCh.sendMessage(new MessageCreateBuilder()
                        .setComponents(Container.of(TextDisplay.of(part1),
                                Separator.createDivider(Separator.Spacing.LARGE), TextDisplay.of(part2)))
                        .useComponentsV2(true).build()).queue();
            sendToCmdChannel(guild, state.requesterCmdChannelId,
                    "✅ قبل تيم **" + state.targetTeamName + "** التحالف! تم إرسال الإعلان الرسمي.");
            ...
            sendToCmdChannel(guild, state.requesterCmdChannelId,
                    "❌ رفض تيم **" + state.targetTeamName + "** طلب التحالف.");
    ...
    private void sendToCmdChannel(Guild guild, String channelId, String msg) {
        ...
        if (ch != null)
            ch.sendMessage(msg).queue(null, e -> {
            });
    }
    ...
    private void editVoteMessageToResult(Guild guild, AllianceVoteState state, boolean accepted) {
        ...
        if (state.rejectVoters.isEmpty())
            ...
            for (String id : state.rejectVoters)
                sb.append("> <@").append(id).append(">\n");
        ch.editMessageById(state.voteMessageId, new MessageEditBuilder()
                .setComponents(Container.of(TextDisplay.of(sb.toString()))).useComponentsV2(true).build())
                .queue(null, e -> {
                });
    }
    ...
    public void onStringSelectInteraction(StringSelectInteractionEvent event) {
        ...
        if (id.startsWith("tm_war_select_")) {
            if (member == null || (!hasLeaderRole(member) && !hasCoLeaderRole(member))) {
                event.reply("❌ ليس لديك صلاحية.").setEphemeral(true).queue();
                return;
            }
            handleWarTargetSelected(event, Integer.parseInt(id.replace("tm_war_select_", "")));
            return;
        }
        if (id.startsWith("tm_ally_select_")) {
            ...
            handleAllianceTargetSelected(event, Integer.parseInt(id.replace("tm_ally_select_", "")));
            return;
        }
        if (id.startsWith("tm_ldr_coset_select_")) {
            if (member == null || !hasLeaderRole(member)) {
                event.reply("❌ فقط الليدر يمكنه تعيين الكو-ليدر.").setEphemeral(true).queue();
                return;
            }
            handleCoLeaderSelected(event, Integer.parseInt(id.replace("tm_ldr_coset_select_", "")));
            return;
        }
        if (!id.startsWith("tm_select_"))
            return;
        ...
        if (id.equals("tm_select_delete")) {
            event.replyComponents(Container.of(
                    ...
                    Separator.createDivider(Separator.Spacing.SMALL),
                    ActionRow.of(Button.danger("tm_confirm_delete_" + teamId, "✅ نعم، احذف"),
    ...
    public void onModalInteraction(ModalInteractionEvent event) {
        ...
        if (id.startsWith("tm_modal_war_")) {
            ...
            if (parts.length >= 2)
                handleWarModal(event, Integer.parseInt(parts[0]), Integer.parseInt(parts[1]));
        } else if (id.startsWith("tm_modal_ldr_color_")) {
            handleColorModal(event, Integer.parseInt(id.replace("tm_modal_ldr_color_", "")));
        } else if (id.startsWith("tm_modal_ldr_name_")) {
            handleNameModal(event, Integer.parseInt(id.replace("tm_modal_ldr_name_", "")));
    ...
    private void handleEditInfoModal(ModalInteractionEvent event, int teamId) {
        String newName = event.getValue("tm_name").getAsString().trim(),
                newColor = event.getValue("tm_color").getAsString().trim();
        event.deferReply().setEphemeral(true).queue();
        TeamData td = getTeamById(teamId);
        if (td == null) {
            event.getHook().editOriginal("❌ الفريق غير موجود!").queue();
        ...
        if (td.roleId != null) {
            ...
            if (r != null)
                r.getManager().setName(newName).setColor(color).queue(null, e -> {
                });
        }
        if (td.categoryId != null) {
            Category cat = guild.getCategoryById(td.categoryId);
        ...
        if (td.voiceChannelId != null) {
            ...
            if (vc != null)
                vc.getManager().setName("🔊・" + newName + "・voice").queue(null, e -> {
                });
        }
        if (td.textChannelId != null) {
            TextChannel tc = guild.getTextChannelById(td.textChannelId);
            if (tc != null)
                tc.getManager().setName("💭・" + newName).queue(null, e -> {
                });
        }
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
                PreparedStatement ps = conn
                        .prepareStatement("UPDATE teams SET name = ?, color = ?, tag = 'Modified' WHERE id = ?")) {
            ps.setString(1, newName);
        ...
        syncToSupabase(newName, fColor, td.leaderId, td.member2Id, td.member3Id, td.member4Id, "Modified",
                oldName.equals(newName) ? null : oldName);
        ...
        updateAnnouncementEmbed(guild, teamId, newName, extractIdOnly(td.leaderId), extractIdOnly(td.member2Id),
                extractIdOnly(td.member3Id), extractIdOnly(td.member4Id), fColor);
        sendLog(guild, "Edit Team Info", event.getUser(), newName,
                "### ✏️ تم تعديل معلومات الفريق\n▫️ **الاسم القديم:** " + td.name + "\n▫️ **الاسم الجديد:** " + newName
    ...
    private void handleEditMembersModal(ModalInteractionEvent event, int teamId) {
        ...
        String m3Id = m3Raw.isEmpty() ? null : extractUserId(m3Raw),
                m4Id = m4Raw.isEmpty() ? null : extractUserId(m4Raw);
        if (leaderId == null) {
        ...
        if (m2Id == null) {
            event.getHook().editOriginal("❌ العضو الثاني إلزامي ولا يمكن حذفه!").queue();
            return;
        }
        Guild guild = event.getGuild();
        String BANNED_ROLE = "1513569599155867678";
        ...
        Member m3M = m3Id != null ? guild.getMemberById(m3Id) : null,
                m4M = m4Id != null ? guild.getMemberById(m4Id) : null;
        if (lM != null && lM.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 1;
        else if (m2M != null && m2M.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 2;
        else if (m3M != null && m3M.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 3;
        else if (m4M != null && m4M.getRoles().stream().anyMatch(r -> r.getId().equals(BANNED_ROLE)))
            bannedNum = 4;
        if (bannedNum != -1) {
            sendLog(guild, "Team Edit Members (Blocked)", event.getUser(), td.name,
                    "### 🚫 محاولة تعديل بعضو محظور\n▫️ **الرقم المحظور:** " + bannedNum, "#ff0000");
        ...
        if (!oldIds.contains(leaderId))
            actuallyNewIds.add(leaderId);
        if (m2Id != null && !oldIds.contains(m2Id))
            actuallyNewIds.add(m2Id);
        if (m3Id != null && !oldIds.contains(m3Id))
            actuallyNewIds.add(m3Id);
        if (m4Id != null && !oldIds.contains(m4Id))
            actuallyNewIds.add(m4Id);
        ...
        Role teamRole = td.roleId != null ? guild.getRoleById(td.roleId) : null,
                leaderRole = guild.getRoleById(LEADER_ROLE_ID);
        ...
        for (String oldId : oldIds) {
            if (!newIds.contains(oldId)) {
                ...
                if (mc != null)
                    ptero.sendConsoleCommand("teama leave " + mc);
            }
        }
        String lMc = getMcName(leaderId);
        if (lMc != null) {
            ptero.sendConsoleCommand("teama join " + td.name + " " + lMc);
            ptero.sendConsoleCommand("teama setowner " + lMc);
        }
        if (m2Id != null) {
            String mc = getMcName(m2Id);
            if (mc != null)
                ptero.sendConsoleCommand("teama join " + td.name + " " + mc);
        }
        if (m3Id != null) {
            String mc = getMcName(m3Id);
            if (mc != null)
                ptero.sendConsoleCommand("teama join " + td.name + " " + mc);
        }
        if (m4Id != null) {
            String mc = getMcName(m4Id);
            if (mc != null)
                ptero.sendConsoleCommand("teama join " + td.name + " " + mc);
        }
        ...
        sendLog(guild, "Edit Team Members", event.getUser(), td.name,
                "### 👥 تم تعديل أعضاء الفريق\n▫️ **القائد الجديد:** <@" + leaderId + ">", td.color);
    ...
    private void sendAnnouncementAndSave(Guild guild, String teamName, String leaderId, String m2Id, String m3Id,
        ...
        ch.sendMessage(new MessageCreateBuilder()
                ...
                .useComponentsV2(true).build()).queue(msg -> {
                    try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
                            PreparedStatement ps = conn
                                    .prepareStatement("UPDATE teams SET announcement_message_id = ? WHERE id = ?")) {
    ...
    private void updateAnnouncementEmbed(Guild guild, int teamId, String teamName, String leaderId, String m2Id,
            String m3Id, String m4Id, String colorCode) {
        TextChannel ch = guild.getTextChannelById(ANNOUNCE_CHANNEL_ID);
        if (ch == null)
            return;
    ...
    public static Container buildAnnouncementContainer(String teamName, String leaderId, String m2Id, String m3Id,
        ...
        String regId = "REG-" + (cleanName.isEmpty() ? "TEAM"
                : cleanName.toUpperCase().substring(0, Math.min(6, cleanName.length())));
    ...
    private void handleDeleteModal(ModalInteractionEvent event, int teamId) {
        ...
        if (leaderRole != null && td.leaderId != null)
            guild.retrieveMemberById(extractIdOnly(td.leaderId))
                    .queue(m -> guild.removeRoleFromMember(m, leaderRole).queue(null, e -> {
                    }), e -> {
                    });
        Role coRole = guild.getRoleById(CO_LEADER_ROLE_ID);
        if (coRole != null && td.coLeaderId != null && !td.coLeaderId.isEmpty())
            guild.retrieveMemberById(td.coLeaderId).queue(m -> guild.removeRoleFromMember(m, coRole).queue(null, e -> {
            }), e -> {
            });
        ...
        // DM للليدر
        if (td.leaderId != null)
            guild.retrieveMemberById(extractIdOnly(td.leaderId)).queue(
                    m -> m.getUser().openPrivateChannel().queue(pc -> pc.sendMessage(deleteMsg).queue(null, e -> {
                    }), e -> {
                    }), e -> {
                    });
        // DM للكو-ليدر
        if (td.coLeaderId != null && !td.coLeaderId.isEmpty())
            guild.retrieveMemberById(td.coLeaderId).queue(
                    m -> m.getUser().openPrivateChannel().queue(pc -> pc.sendMessage(deleteMsg).queue(null, e -> {
                    }), e -> {
                    }), e -> {
                    });
        sendLog(guild, "Delete Team", event.getUser(), td.name,
    ...
    private void syncToSupabase(String name, String color, String leader, String m2, String m3, String m4, String tag,
            String oldName) {
        com.highcore.bot.database.SupabaseManager supa = LeonTrotskyBot.getSupabaseManager();
        if (supa == null)
            return;
    ...
    private void deleteDiscordResources(Guild guild, TeamData td, Runnable onComplete) {
        if (td.roleId != null) {
            Role r = guild.getRoleById(td.roleId);
            if (r != null)
                r.delete().queue(null, e -> {
                });
        }
        if (td.textChannelId != null) {
            TextChannel tc = guild.getTextChannelById(td.textChannelId);
            if (tc != null)
                tc.delete().queue(null, e -> {
                });
        }
        if (td.voiceChannelId != null) {
            VoiceChannel vc = guild.getVoiceChannelById(td.voiceChannelId);
        ...
        if (td.cmdChannelId != null) {
            TextChannel cc = guild.getTextChannelById(td.cmdChannelId);
            if (cc != null)
                cc.delete().queue(null, e -> {
                });
        }
        if (td.logChannelId != null) {
            TextChannel lc = guild.getTextChannelById(td.logChannelId);
            if (lc != null)
                lc.delete().queue(null, e -> {
                });
        }
        if (td.categoryId != null) {
            new Thread(() -> {
    ...
    private void removeRolesFromOldMembers(Guild guild, TeamData td, Role teamRole, Role leaderRole) {
        String[] ids = { extractIdOnly(td.leaderId), extractIdOnly(td.member2Id), extractIdOnly(td.member3Id),
                extractIdOnly(td.member4Id) };
        for (String uid : ids) {
            if (uid == null)
                continue;
            guild.retrieveMemberById(uid).queue(m -> {
                if (teamRole != null)
                    guild.removeRoleFromMember(m, teamRole).queue(null, e -> {
                    });
    ...
    private void addRoleToMember(Guild guild, String userId, Role role) {
        if (userId == null || role == null)
            return;
        guild.retrieveMemberById(userId).queue(m -> guild.addRoleToMember(m, role).queue(null, e -> {
        }), e -> {
        });
    }
    ...
    private String formatDbUser(Member m) {
        return m == null ? null : m.getUser().getName() + " | " + m.getId();
    }
    ...
    private void sendLog(Guild guild, String action, net.dv8tion.jda.api.entities.User user, String target,
            String details, String colorCode) {
    ...
    private String resolveDisplay(Guild guild, String userId) {
        ...
        if (guild != null) {
            ...
            if (m != null)
                return "`" + m.getUser().getEffectiveName() + "`";
        }
        return "<@" + userId + ">";
    }
    ...
    private int getTeamCount() {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
            ...
            logger.error("Error getting team count", e);
        }
        return 0;
    }
    ...
    private List<TeamData> getTeamList() {
        ...
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
                ...
                ResultSet rs = ps.executeQuery()) {
            while (rs.next())
                list.add(TeamData.from(rs));
        } catch (Exception e) {
            logger.error("Error getting team list", e);
        }
        return list;
    }
    ...
    private TeamData getTeamById(int id) {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return TeamData.from(rs);
        } catch (Exception e) {
            logger.error("Error getting team by id", e);
        }
        return null;
    }
    ...
    private TeamData getTeamByName(String name) {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
            ...
            logger.error("Error getting team by name", e);
        }
        return null;
    }

    private TeamData getTeamByLeaderId(String userId) {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
                PreparedStatement ps = conn.prepareStatement("SELECT * FROM teams WHERE leader_id LIKE ?")) {
            ps.setString(1, "%" + userId + "%");
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return TeamData.from(rs);
        } catch (Exception e) {
            logger.error("Error getting team by leader id", e);
        }
        return null;
    }

    private TeamData getTeamByCoLeaderId(String userId) {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
                PreparedStatement ps = conn.prepareStatement("SELECT * FROM teams WHERE co_leader_id = ?")) {
            ps.setString(1, userId);
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return TeamData.from(rs);
        } catch (Exception e) {
            logger.error("Error getting team by co-leader id", e);
        }
        return null;
    }
    ...
    private String getAnnouncementMsgId(int teamId) {
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection();
                PreparedStatement ps = conn
                        .prepareStatement("SELECT announcement_message_id FROM teams WHERE id = ?")) {
            ps.setInt(1, teamId);
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return rs.getString("announcement_message_id");
        } catch (Exception e) {
            logger.error("Error getting announcement msg id", e);
        }
        return null;
    }
    ...
    private static class LiveTeamStats {
        int level = -1;
        int points = -1;
        int wars = -1;
        int deaths = -1;
        int claims = -1;
        int generators = -1;
        String enemies = "لا توجد";
        String allies = "لا يوجد";
    }

    private LiveTeamStats fetchLiveTeamStats(String teamName) {
        ...
        try (Connection conn = LeonTrotskyBot.getDbManager().getConnection()) {
            ...
            try (PreparedStatement ps = conn
                    .prepareStatement("SELECT COUNT(*) as c FROM generators WHERE team_name = ?")) {
                ps.setString(1, teamName);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next())
            ...
            logger.error("Error fetching live stats", e);
        }
        return stats;
    }
    ...
    private String checkSingleIdOnline(String id, String mention) {
        ...
        if (!isOnline) {
            return mention + " (`" + mcName
                    + "`) ليس متواجداً داخل السيرفر حالياً! يجب أن يكون الأعضاء الجدد بداخل اللعبة.";
        }
        return null;
    }
    ...
    private static class TeamData {
        int id;
        String name, color, roleId, categoryId, voiceChannelId, textChannelId, cmdChannelId, logChannelId;
        ...
        static TeamData from(ResultSet rs) throws SQLException {
            ...
            try {
                t.cmdChannelId = rs.getString("cmd_channel_id");
            } catch (Exception ignored) {
            }
            try {
                t.logChannelId = rs.getString("log_channel_id");
            } catch (Exception ignored) {
            }
            try {
                t.coLeaderId = rs.getString("co_leader_id");
            } catch (Exception ignored) {
            }
            try {
                t.logoUrl = rs.getString("logo_url");
            } catch (Exception ignored) {
            }
            return t;
        }
